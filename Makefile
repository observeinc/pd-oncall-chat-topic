...
BUCKET=obs-$(STACKNAME_BASE)
SSMKeyArn=$(shell aws --profile $(PROFILE) kms --region $(REGION) describe-key --key-id alias/aws/ssm --query KeyMetadata.Arn)
...
deploy:
	cd lambda && \
		...
		aws --profile $(PROFILE) s3 cp --region $(REGION) /tmp/deployment.zip \
			s3://$(BUCKET)/$(MD5) && \
